...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "70ce3bef-3107-4cdd-8101-93ba05f3e380"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.7622574438968169"
...
#  name        = "hronaspdotnet-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application hronaspdotnet created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for hronaspdotnet and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "hronaspdotnet-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "hronaspdotnet"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_hronaspdotnet"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "hronaspdotnet instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/hr-onaspdotnet:latest",
      "sudo docker run -d -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/hronaspdotnet theharbormaster/hr-onaspdotnet:latest"
    ]
  }
}
